...
`tailwindcss-language-server` directory. The package is a dependency-free esbuild bundle that
carries the prebuilt `@parcel/watcher` binaries for every platform, so unlike the Lua, Bazel, and
TypeScript samples this plugin needs no native variants: one distribution ZIP works everywhere.
...
keeps resolution independent of the working directory, which is the opened project — without it npx
would run the project's own `tailwindcss-language-server` when one is installed there.

> [!NOTE]
> `withNodeRuntimeEnsured` and `LspNodeRuntimeManager` live in `com.intellij.platform.lsp.impl.nodeRuntime`
> and require a recent IntelliJ IDEA 263 EAP build, hence `sinceBuild = "263"`.

To move to a newer server release, change `tailwindServerVersion` in
