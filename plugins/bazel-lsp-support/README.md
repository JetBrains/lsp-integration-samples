...
This module implements an IntelliJ Platform plugin that integrates
[bazel-lsp](https://github.com/cameron-martin/bazel-lsp) with IntelliJ IDEA
through the [LSP API](https://plugins.jetbrains.com/docs/intellij/language-server-protocol.html).

The `bazel-lsp` binaries are downloaded from the project's GitHub releases at build time
(see the `ivy` repository in [settings.gradle.kts][file:settings.gradle.kts]) and bundled into
platform-specific plugin distributions produced by the `buildPluginVariants` task.
The language server handles `BUILD`, `WORKSPACE`, `*.bazel`, `*.bzl`, `*.star`,
and `*.sky` files.

> [!NOTE]
> `bazel-lsp` invokes the `bazel` executable to resolve labels and external repositories,
> so `bazel` (or `bazelisk` aliased as `bazel`) must be available on the `PATH`.

> [!NOTE]
> `buildPluginVariants` always emits an archive for every OS/architecture pair, but
> upstream publishes no `windows-arm64` binary, so the `windows-arm64` archive contains
> no language server and is not published by CI.
